...
function buildChartSection(catData) {
    ...
    let colorPallette = ['#33DFA1', '#384B42', '#9BB0A5', '#00D2FF', '#009CF6']; //lime-cream shade
    ...
    let layout = {
        ...
        font: {weight: 'bold', size: 18, color: 'black'},        
        height: height,
        width: 400,
        showlegend: false,
...
function buildMapSection(catData) {
    ...
    for (i=0; i < catData.length; i++) {
        ...
        let new_icon = L.divIcon({
            className: 'custom-marker',
            html: '<div class="marker-label">' + catData[i]['rank'] + '</div>',
            iconSize: [30, 30]
        });
        let locMarker = L.marker([lat, lon], {icon: new_icon});
